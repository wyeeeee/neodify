...
describe('App Login Page', () => {
  ...
  test('should delete agent from action menu', async () => {
    let deleted = false
    const fetchMock = vi.fn().mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/agents/agent-main') && (!init?.method || init.method === 'GET')) {
        return {
          ok: true,
          text: async () =>
            JSON.stringify({
      ...
      if (url.endsWith('/agents/agent-main') && init?.method === 'DELETE') {
        deleted = true
        return {
          ok: true,
    ...
    const actionButton = wrapper.find('button.agent-action-trigger')
    expect(actionButton.exists()).toBe(true)
    await actionButton.trigger('click')
    await flushPromises()

    const deleteButton = document.body.querySelector('button.agent-action-item.is-danger') as HTMLButtonElement | null
    expect(deleteButton).toBeTruthy()
    deleteButton?.click()
    await flushPromises()

    const confirmDeleteButton = document.body.querySelector('button.agent-action-item.is-danger') as HTMLButtonElement | null
    expect(confirmDeleteButton).toBeTruthy()
    confirmDeleteButton?.click()
    await flushPromises()

    const hasDeleteCall = fetchMock.mock.calls.some((call) => {
      const url = String(call[0])
      const method = (call[1] as RequestInit | undefined)?.method
      return url.endsWith('/agents/agent-main') && method === 'DELETE'
    })
